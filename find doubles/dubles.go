...
func main() {
	...
	file, err := os.Create("find doubles/doubles.csv")
	...
	for i, curi := range result{
		for j,curj := range  result{
			if i != j {
				for _, cur := range append([]string{curi.Name},curi.Synonyms...){
					if stringInSlice (cur,append([]string{curj.Name},curj.Synonyms...)){
						//fmt.Printf("совпадение: в метаболитах (%s %s) : %s",curi.Accesid,curj.Accesid, cur)
						err := writer.Write([]string{curi.Accesid,curj.Accesid,cur})
						checkError("Cannot write to file", err)
					}
				}
			}
